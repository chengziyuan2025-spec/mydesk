...
export interface ExternalSearchData { systemApps?: SystemAppCatalogItem[]; everything?: EverythingSearchItem[] }
const normalize = (value: string) => value.trim().toLocaleLowerCase();
const normalizePath = (value: string) => value.replaceAll("/", "\\").toLocaleLowerCase();
const pinyinTokens = (value: string) => !/[\u3400-\u9fff]/.test(value) ? [] : [
...
function textScore(title: string, aliases: string[], extra: string[], needle: string) {
  ...
  return Math.max(aliasScore, ...[title, ...pinyinTokens(title), ...extra].map(normalize).map((value, index) => fuzzyScore(value, needle) - index * 25));
}
...
export function searchDeskBox(data: AppData, query: string, limit = 30, external: ExternalSearchData = {}): SearchResult[] {
  ...
  if (!fileOnly) {
    for (const container of data.containers) {
      const score = needle ? textScore(container.name, container.aliases, [], needle) : 0;
      ...
      for (const shortcut of container.shortcuts) {
        const shortcutScore = needle ? textScore(shortcut.name, shortcut.aliases, [shortcut.path, container.name], needle) : 0;
    ...
    for (const item of external.systemApps ?? []) {
      ...
      const score = needle ? textScore(item.name, entry?.aliases ?? [], [item.target], needle) : 0;
  ...
  for (const item of external.everything ?? []) {
    const entry = persisted.get(item.key) ?? null;
    const score = needle ? textScore(item.name, entry?.aliases ?? [], [item.path], needle) : 0;
  ...
  for (const entry of data.externalLauncherEntries.filter((item) => !results.some((result) => result.id === item.key))) {
    ...
    const score = needle ? textScore(entry.name, entry.aliases, [entry.target], needle) : 0;
  ...
  const calculation = calculate(cleaned);
